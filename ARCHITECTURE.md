...
メインサイト（hiroakiishibashi.com）はゲームページに**共通ヘッダー**（`/js/common-header.js`：Home/Ranking/About・JP/EN・Login/Profile、sticky 60px）を載せる。連携仕様は web リポジトリ `games/yuugure/HANDOFF.md`。**衝突の経緯**: YUUGURE の dist を `games/yuugure/` に直接 push していたためポータル側ラッパーが消える事故 → 下記で恒久対応。

- **ビルド出力に共通ヘッダーを内包**: `index.html` の `<body>` 先頭に `<nav><!-- replaced by /js/common-header.js --></nav>` プレースホルダ。`/js/common-header.js` の `<script>` は **vite.config の `transformIndexHtml` プラグイン（`yuugure-portal-header`）で注入**。
  - 直接 index.html に `<script src="/js/common-header.js">` を書くと Vite がビルド時に解決を試みて失敗する（このリポジトリには無くポータルにのみ存在）ため、タグ注入で回避。
  - これで `dist/index.html` が自己完結 → ポータルの `rsync --delete dist/ → games/yuugure/` 同期でヘッダーが消えない。
  - ローカル/単体配信では `/js/common-header.js` は404 → ヘッダー無しでゲームのみ動作（無害）。
- **レイアウト**: body を flex から通常フローに変更し `#app{margin:0 auto}` で中央寄せ。sticky ヘッダー（in-flow）の下にゲームが並ぶ（padding-top 不要）。
...
- **連携フロー（HANDOFF）**: YUUGURE 側= src編集＋`npm run build`→`dist/`＋完了報告。メインサイト統合側= `rsync --delete dist/ → games/yuugure/`＋git push。**web リポジトリの `games/yuugure/` は YUUGURE 側から直接 push しない**（二重書き込み衝突防止）。
- 検証: prod `dist/index.html` に `<nav>`＋`/js/common-header.js`＋`/games/yuugure/assets/...`。ローカル(base /)配信でゲーム正常描画（中央寄せ・ヘッダーは404で非表示）。tsc / vitest 79件 / build OK。
