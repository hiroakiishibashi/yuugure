...
## 部屋の中／外 シーン切り替え + スライドドア トランジション（2026-06-01）

画面上部の「シーン」が2つの空間を切り替える構成に再設計。

- **2モード（`GameController.sceneMode`）**:
  - `inside`（部屋の中）= 永続 PixiJS の部屋＋キャラ＋HUD（「🚪へやを でる」「🛠もようがえ」）。
  - `outside`（部屋の外）= 青空アパート DOOR SELECT（ドア＋階移動＋出入り）。
  - `subscribeMode`/`subscribeEdit` で React が購読。`enterRoom`(旧visitRoom)=insideへ＋訪問(LIVE・編集不可)、`goHome`=自室insideへ(吹き出しclear)、`goOutside`=outsideへ。
- **遷移演出**: `GameProvider` がモード変更を検知し**スライドドア curtain**を再生（左右の扉が閉じる→裏でレイヤーswap→開く、CLOSE 320ms+SWAP 90ms）。canvasは常時マウント（display切替）でGLコンテキスト維持。
- **編集モード（もようがえ）**: 自室のみトグル表示（`isOwnRoom`）。ONで `RoomRenderer.setEditable(true)` → 家具がドラッグ可能に＋持ち物パレット(`ItemPanel`)表示。訪問中は編集不可（家具は読み取り専用）。
- **下部パネル(App)**: 空間系(ドア/部屋)をシーンへ移動したため、タブは「NMLの小箱」「じぶんの小箱」の2つに。`RoomView.tsx` は HUD(`InsideControls`)へ統合し削除。
- 検証: ブラウザで 自室(intro吹き出し)→でる(扉閉→開)→outside(A501–A506)→A503入室(LIVE+のがみ+吹き出し, でるのみ)→でる→じぶんのへや(吹き出しclear+もようがえ復活)→もようがえON(パレット)→ライト設置(4→3)。tsc clean / vitest 79件 / build OK。
...
## ポータル共通ヘッダー統合 + ビルド運用（2026-06-01）

メインサイト（hiroakiishibashi.com）はゲームページに**共通ヘッダー/フッター**（`/js/common-header.js`：Home/Ranking/About・JP/EN・Login/Profile、sticky 60px / `/js/common-footer.js`）を載せる。連携仕様は web リポジトリ `games/yuugure/HANDOFF.md`。**衝突の経緯**: YUUGURE の dist を `games/yuugure/` に直接 push していたためポータル側ラッパーが消える事故 → 下記で恒久対応。

- **ビルド出力に共通ヘッダー/フッターを内包**: `index.html` の `<body>` 先頭に `<nav><!-- replaced by /js/common-header.js --></nav>` プレースホルダ。`/js/common-header.js` と `/js/common-footer.js` の `<script>` は **vite.config の `transformIndexHtml` プラグイン（`yuugure-portal-chrome`）で注入**。
  - 直接 index.html に `<script src="/js/common-header.js">` を書くと Vite がビルド時に解決を試みて失敗する（このリポジトリには無くポータルにのみ存在）ため、タグ注入で回避。
  - これで `dist/index.html` が自己完結 → ポータルの `rsync --delete dist/ → games/yuugure/` 同期でヘッダー/フッターが消えない。
  - ローカル/単体配信では `/js/common-header.js`/`common-footer.js` は404 → ゲーム単体で動作（無害）。
- **レイアウト**: body を flex から通常フローに変更し `#app{margin:0 auto}` で中央寄せ。sticky ヘッダー（in-flow）の下にゲームが並び、フッターはゲーム下に入る（padding-top 不要）。
- **base 運用**: `npm run build` = `YUUGURE_BASE=/games/yuugure/`（本番＝ポータル正）/ `npm run build:local` = root base（ローカル静的プレビュー）。`vite.config` の base 既定は `/`（テストの BASE_URL を素のまま保つ — 既定を変えると assetPaths/characters テストが落ちる）。
- **連携フロー（HANDOFF）**: YUUGURE 側= src編集＋`npm run build`→`dist/`＋完了報告。メインサイト統合側= `rsync --delete --exclude HANDOFF.md dist/ → games/yuugure/`＋git push。**web リポジトリの `games/yuugure/` は YUUGURE 側から直接 push しない**（二重書き込み衝突防止）。
- 検証: prod `dist/index.html` に `<nav>`＋`/js/common-header.js`＋`/js/common-footer.js`＋`/games/yuugure/assets/...`。ローカル(base /)配信でゲーム正常描画（中央寄せ・共通JSは404で非表示）。tsc / vitest 79件 / build OK。

---

## 企画書/原作UI準拠リビルド（2026-06-06）

`これまでの企画書/夕暮れVer.0.4.ppt`・`これまでの企画書/のけアプリ.ppt` と、gdrive `visual/screen_shot` の原作GIFを再確認し、現在の独自UIを原作の「夕暮れの部屋」構成へ戻す方向で再設計した。

- **企画書から復元した前提**: 公式HP/青空不動産/青空アパート/大家掲示板/What’s New/NML投稿・評価・配布/のけアプリ（`.nml` 再生・作成・ドラッグ投入）を中心にした、アパート+NMLコミュニティの導線。アプリ内では部屋訪問、NMLの小箱、しあわせの箱、部屋の模様替えが主導線。
- **画面サイズ/構造**: 上段を原作スクショ準拠の `640x271` ゲーム画面へ戻し、下段を `自分の部屋` / `NMLの小箱` / `しあわせの箱` の小箱UIに変更。大きなReactヘッダーや現代的カード構造を撤去し、ポータル共通ヘッダー下で横幅640pxのページとして収まるようにした。
- **部屋の中**: `PixiHost` を `640x271` に再構成し、青グラデ+`visual/room/BG1.png` の原作部屋背景、等倍ピクセルアート、LIVE表示、頭上吹き出し、初期家具配置を実装。自室は `9999号室 じぶんの部屋` として家具を持つ。訪問部屋はLIVE+住人名+読み取り専用。
- **部屋の外**: `DoorGallery` を DOOR SELECT 画面に寄せ、`A501-A506` の青ドア画像、階移動、地区/空室表示、覗き窓キャラ、ドアクリック入室に再構築。`A101-A106` も追加し、原作スクショの `A105号室 伊東四郎さんの部屋` を再現対象に入れた。
- **家具/編集**: gdrive `visual/room` から約100点の `item_*.png` と `BG1.png` を取り込み、`ROOM_ITEMS` を原作家具カタログへ拡張。従来IDは互換aliasとして残し、スターター所持品を原作アイテム中心に拡張。自室のみ編集ONで家具パレット表示+配置+ドラッグ移動、訪問先は閲覧のみ。
- **NMLの小箱**: 原作のメールビューに合わせ、宛名/件名/差出人/評価/日時/NEW!/`n/999通`/まえ/つぎ/削除/閉じる/部屋訪問ボタンの構成へ変更。
- **起動安定性**: Supabase `getSession`/scores query に短いタイムアウトを入れ、オフライン/ヘッドレス検証時にゲーム本体が `よみこみ中` で止まらないようにした。未ログイン時は従来通り localStorage fallback。
- **検証**: `npx tsc --noEmit`、`npm test -- --run`（79件）、`npm run build:local`、Chrome CDPスクリーンショット（自室/DOOR SELECT/訪問部屋）で確認。`dist/` は生成済みだが、web repo への同期/push は HANDOFF に従いメインサイト統合スレッド担当。
